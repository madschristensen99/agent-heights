...
Agent HQ is a retro top-down office game where you hire real AI coding agents — each employee at a desk is a live Cline SDK agent routed through the Swarms API that actually reads, writes, and runs code in its own sandboxed workspace folder while you watch from a Phaser game world.

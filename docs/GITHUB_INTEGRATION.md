# Agent Heights — GitHub Integration & Deployment Pipeline

## The Problem

Agent Heights agents produce real code, but that code goes nowhere. Work lives in
...
created outside Agent Heights. Agents can query deployments and check logs, but they
...
Agent Heights operates as a managed platform. The server holds a GitHub App (or bot
...
```
github.com/agent-heights-org/user-{userId}
  └── main
...
```
github.com/agent-heights-org/{userId}-{projectName}
```
...
- `shared/types.ts` — Add `github` to `GameSettings`:
  ...
  github: {
    enabled: boolean;
    org: string;       // GitHub org name (e.g. "agent-heights-users")
...
- **Search** — Free-text search across all of GitHub. Results show repo name,
  ...
  workspace" button.
- **My Repos** — Shows repos in the user's Agent Heights org. Lists branches, open
...
2. Server forks the repo to the user's Agent Heights org

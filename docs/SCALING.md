# Agent HQ — Scaling Roadmap: The System Builds the System

> Agent HQ is not just a product. It's a factory. The HQ HQ is the factory
...
│                      HQ HQ (The Platform)                     │
│                                                              │
│  The master Agent HQ instance. Runs on Railway.              │
...
│       │                                                      │
│       ▼                                                      │
│  Railway: agent-hq-api, agent-hq-redis,                      │
│           agent-hq-worker, agent-hq-db, ...                  │
│                                                              │
└────────────────────────┬────────────────────────────────────┘
                         │
         ┌───────────────┼───────────────┐
...
The meta-office. An Agent HQ instance where the "product" being built IS
Agent HQ. The agents there work on the real codebase, deploy to Railway,
...
What users get when they sign up. Each is a fully isolated Agent HQ office
...
1. **HQ HQ is a special tenant, not a separate codebase.** The HQ HQ is
   ...
   agents work on the Agent HQ codebase (their workspace is a clone of the
...
- Don't run agents on the Agent HQ server at all. Offload to:
...
- **Pros**: True horizontal scaling, agents can't touch the server, works
  across multiple Agent HQ instances.
...
**No new infrastructure needed.** Uses the existing single-process Agent HQ
+ Hermes Railway MCP.
...
| Worker "database" | "Write a Supabase migration that adds `tenant_id` to the `agent_hq_saves` table. Add RLS policies so users can only access rows where `tenant_id = auth.uid()`. Keep the JSONB blob for now — we'll decompose it in Phase 2." |
...
| Agent | Task |
|---|---|
| Hermes | "Deploy a Redis service on Railway. Create a service called `agent-hq-redis`. Get the connection URL and set it as an env var on the main service." |
...
- Multiple Agent HQ server instances can share state — any server can
...
| Agent | Task |
|---|---|
| Hermes | "Deploy a new Railway service called `agent-hq-worker`. It's a Node process that accepts task requests via HTTP and runs them in isolated sandboxes. Give it its own volume for agent workspaces." |
...
| Agent | Task |
|---|---|
| Hermes | "Configure a Railway load balancer / reverse proxy in front of the Agent HQ API service. Enable sticky sessions or verify that stateless operation works without stickiness." |
...
**Step 3**: Hermes deploys the updated Agent HQ to Railway using the
Railway MCP tools.

**Step 4**: The updated Agent HQ now has new capabilities. You use it to
...
- **Hermes can deploy** — the Railway MCP integration means Hermes can
  ...
  leave Agent HQ to deploy changes.
- **The workspace is the codebase** — if you point an agent's workspace
  at a clone of the Agent HQ repo, they can work directly on the codebase.
...
The catch-22: to have multiplayer Agent HQ, you need to scale Agent HQ,
and to scale Agent HQ, you're using Agent HQ which isn't scaled yet. But
this is fine because:

1. You're a single user (the developer) using Agent HQ to build Agent HQ.

...
export class BootScene extends Phaser.Scene {
  ...
  async preload(): Promise<void> {
    ...
    // Only load if the world theme has assetTier "ai" (or HQ mode with no theme).
    // Procedural-only worlds skip this — paintRenderRow falls back to world-tiles.
    const theme = this.registry.get("worldTheme") as WorldTheme | null;
    ...
    if (isAiTier) {
      ...
      console.log("[boot] Procedural asset tier — skipping AI atlases");
    }

    // 3D creature spritesheets (8 directions × 4 animation frames, 128px cells)
    // Only load if AI tier — procedural worlds use canvas-drawn creatures.
    if (isAiTier) {
      ...
      console.log("[boot] Procedural asset tier — skipping 3D creature spritesheets");
    }
    ...
    this.load.on("loaderror", (file: Phaser.Loader.File) => {
      // Suppress warnings for 3D creature spritesheets — they may not exist
      // in procedural-tier worlds and the fallback is handled gracefully.
    ...
    // ── World Theme: load theme-specific assets ──────────────────────
    // (world-theme.json was already fetched above to determine assetTier)
    if (theme) {
  ...
  private createAnimations(): void {
    ...
    const sheets = [...Array.from({ length: CHAR_VARIANTS }, (_, i) => `char-${i}`), "char-agent-resources", "char-hermes"];

...
export class OfficeScene extends Phaser.Scene {
  ...
  private tryOfficeInteract(time: number): boolean {
    ...
    const projPx = { x: this.projectorTile.x * TILE_PX + 32, y: this.projectorTile.y * TILE_PX - 48 };
  ...
  private updateInteractHints(time: number): void {
    ...
    // Projector screen
    const projHintPx = { x: this.projectorTile.x * TILE_PX + 32, y: this.projectorTile.y * TILE_PX - 48 };
  ...
  private drawProjector(): void {
    ...
    const py = this.projectorTile.y * TILE_PX - 48;
    const sw = 480;
    const sh = 288;

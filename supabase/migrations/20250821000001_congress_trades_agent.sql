...
-- The new agent uses our own MCP server (server/providers/congress-trades-mcp.ts) that fetches
-- data from CongressInvests.com free API (100 req/day, no key needed).
-- Covers both House and Senate PTR filings from official government sources.
...
Data source: CongressInvests.com free API — aggregates official House and Senate PTR filings from disclosures-clerk.house.gov and efdsearch.senate.gov.
...
To start: Just hire the agent. No authentication required.',
    ...
    'agent',
    'approved',
    '["Get the most recently filed congressional trades","Get trades by stock ticker with chamber filter","Get trades by politician name (partial match)","Filter trades by buy/sell transaction type","Identify buy momentum signals from politician trades"]',

...
CREATE TABLE IF NOT EXISTS public.agent_hq_conversation_messages (
...
ALTER TABLE public.agent_hq_conversation_messages ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users read own conversation messages"
  ON public.agent_hq_conversation_messages FOR SELECT
  USING (auth.uid() = owner_id);

CREATE POLICY "Users insert own conversation messages"
  ON public.agent_hq_conversation_messages FOR INSERT
  WITH CHECK (auth.uid() = owner_id);

CREATE POLICY "Users delete own conversation messages"
  ON public.agent_hq_conversation_messages FOR DELETE
  USING (auth.uid() = owner_id);

CREATE POLICY "Service role full access to conversation messages"
  ON public.agent_hq_conversation_messages FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

CREATE INDEX IF NOT EXISTS idx_conversation_messages_agent
  ON public.agent_hq_conversation_messages (agent_id, seq);
CREATE INDEX IF NOT EXISTS idx_conversation_messages_owner
  ON public.agent_hq_conversation_messages (owner_id);

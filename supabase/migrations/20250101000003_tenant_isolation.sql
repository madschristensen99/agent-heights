...
-- The existing agent_hq_saves table already uses user_id as PRIMARY KEY with RLS
...
ALTER TABLE public.agent_hq_saves
  ADD COLUMN IF NOT EXISTS tenant_id UUID;

-- Backfill existing rows: tenant_id = user_id
UPDATE public.agent_hq_saves
...
CREATE INDEX IF NOT EXISTS idx_agent_hq_saves_tenant_id
  ON public.agent_hq_saves (tenant_id);
...
CREATE POLICY "Service role full access to HQ saves"
  ON public.agent_hq_saves FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
